...
For getting started, shell commands, and how-to guides, see [USER\_MANUAL.md](USER_MANUAL.md).
...
| 1 | User-facing tasks (run\_demo, test\_all) |
...
Tags: `01`\=SVC, `02`\=SVC\_RET, `10`\=SENSOR\_READ, `11`\=SENSOR\_DATA,  
`12`\=KEEPALIVE, `13`\=LUMP\_MSG, `20`\=DELAY, `40`\=DEMO\_START/END,  
...
| RTIC priority | HW byte | Tasks |
| --- | --- | --- |
| 3 | 0xD0 | UART ISRs (sensor) |
| 2 | 0xE0 | USB, heartbeat, motor, sensor, shell |
...
| --- | --- | --- |
| DWT\_CTRL | 0xE0001000 | Comparator count (bits 31:28), cycle counter enable |
| DWT\_COMPn | 0xE0001020 + 16n | Comparator value (address to watch) |
| DWT\_MASKn | 0xE0001024 + 16n | Address mask (power-of-2 alignment) |
| DWT\_FUNCTIONn | 0xE0001028 + 16n | Function select + MATCHED bit |
| DEMCR | 0xE000EDFC | Debug enable: TRCENA, MON\_EN |
| DWT\_LAR | 0xE0001FB0 | Lock Access Register (write 0xC5ACCE55) |

**Watchpoint functions (DWT\_FUNCTIONn bits 3:0):**
...
stub — "Demon mode" runs on Hub — for remote debugging over USB CDC serial.
...
*   `g` reads all 26 ARM registers from saved DebugMonitor context
*   `G` writes all registers (accepted, currently ignored)
...
| Motor position tracking (motor\_poll task) | Done |
| Closed-loop motor\_goto positioning (±9°) | Done |
...
| setjmp/longjmp abort + force\_kill\_sandbox (kill -9) | Done |
...
| GDB continue/step/halt/detach — full debug cycle | Done |
| Watchdog reset detection + chime | Done |
...
_See also:_ [USER\_MANUAL.md](USER_MANUAL.md) for getting started,  
