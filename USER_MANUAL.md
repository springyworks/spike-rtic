...
- [SPIKE-rtic  Hub — User Manual](#spike-rtic--hub--user-manual)
  - [Contents](#contents)
  ...
  - [3 Process Management](#3-process-management)
    ...
    - [3.3 Short Press Behavior ( 1 s)](#33-short-press-behavior--1-s)
...
```bash
# Build a demo ($PROJECT_ROOT is auto-set by .bashrc)
cd $PROJECT_ROOT/examples/hub-ram-demos
cargo build --release

# Extract raw binary (examples/ subdir, underscores in name)
arm-none-eabi-objcopy -O binary \
...
| `motor <a-f> pos`       | Read motor position (works with motorpoll auto-sync) |
...
Send `$D#44` (detach) or Ctrl-C three times to return to the shell.
...
**Quick start — debug pipeline (recommended):**

The `debug_pipeline.py` script does everything in one shot: uploads the
demo binary, sends `go` to start it, sends `gdb` to enter RSP mode, and
...
**VS Code F5** — the workspace has `launch.json` and `tasks.json` configs
...
**In GDB, these commands are verified working:**

```gdb
info registers               # r0-r15, cpsr (valid values)
bt                           # backtrace with source locations
x/4xw 0x20040000            # read SRAM2 (demo code)
continue                     # resume demo execution
# Ctrl-C                     # break into halted state (SIGINT)
watch *(uint32_t*)ADDR       # DWT write watchpoint (4 hw slots)
detach                       # clean exit → hub returns to shell
quit                         # disconnect → auto-cleanup on hub
```

> **Note:** The demo's `delay_ms` uses SVC, which GDB reports as SIGTRAP.
> Just `continue` past it — this is normal.

**Hub state detection** — `hub_state.py` probes the serial port to
classify the hub before acting:

| State code | Name             | Meaning                              |
| ---------- | ---------------- | ------------------------------------ |
| 0          | `disconnected`   | No LEGO USB device found             |
| 1          | `dfu`            | Hub in DFU bootloader (0694:0011)    |
| 2          | `serial-shell`   | Shell responds with `spike>` prompt  |
| 3          | `serial-gdb`     | Hub in GDB RSP mode (no prompt)      |
| 4          | `serial-unknown` | Serial present, no recognizable data |
| 5          | `serial-busy`    | Port locked by another program       |
...
| `imu [init]`    | Init LSM6DS3TR-C IMU, show WHOAMI                            |
...
| `color-seeker`      | `examples/color_seeker.rs`      | v7+ | Rotational color scanner: motorgoto steps + color classify |
...
*See also:* [REFERENCEMANUAL.md](REFERENCE_MANUAL.md) for hardware specs,  
