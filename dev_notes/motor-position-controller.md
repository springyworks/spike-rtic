...
| decel_zone | (= et × 5) | 75 | Ramp-down distance, derived from et |
...
5. **omax controls overshoot vs speed tradeoff**:
...
- Gate PID on data_seq changes (only act on fresh sensor data)
- Expose motor_goto to MonitorApi for user apps (currently shell-only)
