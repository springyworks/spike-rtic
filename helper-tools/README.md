...
**Python** — quick, interactive, great for serial port wrangling,  
...
**xtask** (Rust) — the Rust way. Build, objcopy, upload, debug  
...
| --- | --- | --- |
| **udev symlinks** | `helper-tools/99-spike-hub.rules` creates stable `/dev/spike-shell` and `/dev/spike-gdb` | `test_gdb_proto.py`, manual use |
...
_See also:_ [99-spike-hub.rules](99-spike-hub.rules) for  
udev symlink setup.